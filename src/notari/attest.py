...
This module answers "no" with Ed25519:
  ...
  * **Contracts are signed by a human approver.** The private approver key never
    lives in the repo or on the build machine. ``notari verify`` checks the
  ...
  * **Passports are signed by the gate identity.** The verdict is signed with a
    ...
    re-verify a passport with ``notari verify-passport`` against the gate's public
...
``notari keygen`` and held by the human / the CI identity, off the agent's box.
"""
