...
Storage: `~/.notari/session_approvals/<session_id>.json`. Atomic
...
def _dir() -> Path:
    base = os.environ.get("NOTARI_SESSION_APPROVALS_DIR")
    if base:
        return Path(base)
    return Path.home() / ".notari" / "session_approvals"
...
def forget_session(session_id: str) -> None:
    """Wipe one session's approval cache. Called by `notari session reset`
