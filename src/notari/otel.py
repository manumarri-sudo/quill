...
When `notari[otel]` is installed AND OTel is configured (env var
...
Phoenix / Honeycomb ingest Notari data without inventing a new format.
...
- Optional extra (`notari[otel]`), not default-on. Footprint: 3
...
- `gen_ai.operation.name` = the audit event_type (e.g. "verdict.blocked")
  so observability dashboards can filter by Notari's own taxonomy too.

If the user doesn't `pip install 'notari[otel]'`, the import in this
...
# is visible. `notari doctor` surfaces the count.
...
def _try_load_otel() -> bool:
    ...
    try:
        ...
        _otel_tracer = trace.get_tracer("notari", "0.2.0")
    except ImportError:
        _otel_tracer = None
    return _otel_tracer is not None
...
# ---------------------------------------------------------------------------
# Mapping from Notari audit events to OTel GenAI span names + attributes
...
def _span_attributes(
    ...
    Plus Notari-specific attributes prefixed `notari.*` so dashboards
...
def _provider_name(agent_id: str) -> str:
    """Map Notari's agent_id taxonomy to a GenAI semconv provider name."""
    if agent_id.startswith("upstream:"):
        return agent_id.split(":", 1)[1]
    if agent_id.startswith("claude-code"):
        return "claude-code"
    if agent_id == "notari.notify" or agent_id.startswith("notari."):
        return "notari"
...
def emit_span(
    *,
    ...
    with (
        ...
        # relying on notari.event_type substring matching.
        if event_type in (
            "verdict.blocked",
            "verdict.scope_violation",
            "tool.errored",
