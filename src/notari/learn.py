...
operator. The operator decides whether to apply them. Notari never
...
Suggestion classes:
  ...
  3. False-positive overrides - patterns whose blocks the operator has
     repeatedly bypassed via `notari approve`. Suggests adding a
    ...
    for cwd, n in sorted(asks_by_cwd.items(), key=lambda kv: -kv[1]):
        ...
        out.append(
            Suggestion(
                ...
                paste_command=f"notari trust add {cwd}",
                evidence=(
                    f"asks={n}",
                    f"blocks={n_blocks}",
                ),
...
def analyze_decayed_permissions() -> list[Suggestion]:
    ...
    out: list[Suggestion] = []
    try:
        from notari import decay as _decay
    ...
    for perm in store.decayed():
        out.append(
            Suggestion(
                ...
                paste_command=f"notari decay reaffirm {perm.pattern}",
...
def analyze_false_positive_overrides(
    ...
    for e in events:
        ...
        # reason ends with "via notari approve <token-prefix>"; the
        ...
        out.append(
            Suggestion(
                ...
                rationale=(
                    ...
                    f"it via `notari approve` {n} times. Repeated overrides "
                ...
                paste_command=(
                    f'edit ~/.notari/config.toml and set: [policy]\n  "{tool_name}" = "medium"'
                ),
    ...
    for head, n in sorted(pattern_hits.items(), key=lambda kv: -kv[1]):
        ...
        out.append(
            Suggestion(
                ...
                paste_command=(
                    f"edit ~/.notari/config.toml and add: "
        ...
        out.append(
            Suggestion(
                ...
                rationale=(
                    ...
                    "src/notari/journal.py:summarize_transcript and confirm "
