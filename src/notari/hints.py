...
single hint line under each block that points at one of Notari's other
features (`notari saves`, `notari insights`, `notari audit export --pack`,
etc.). The selector picks based on:
  ...
  2. Risk class               e.g. critical blocks suggest verifying the
     audit chain
  3. State                    e.g. first-ever block prompts `notari
     integrate` to wire up agent-side discovery
  4. Generic rotation         if nothing matches, a generic hint rotates
     in

Cooldowns are per-hint, tracked in `~/.notari/hints_seen.json`, default
...
def _seen_path() -> Path:
    ...
    override = os.environ.get("NOTARI_HOME")
    base = Path(override) if override else (Path.home() / ".notari")
