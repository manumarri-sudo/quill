...
Non-interactive fallback: under `notari serve` (MCP stdio proxy mode), the
...
`notari approve <token>` line on stderr, and declines THIS call. The agent's
retry of the same call within the TTL will be allowed automatically by the
approval-consumption path in `notari.adapters.claude_code.run_hook`.
"""
...
from notari.errors import ConfirmationMismatch, HumanDeclined
from notari.policy import Risk

# Anti-fatigue tunables (env-overridable for testing).
FATIGUE_WINDOW: Final[int] = int(os.environ.get("NOTARI_FATIGUE_WINDOW", "3"))
FATIGUE_THRESHOLD_S: Final[float] = float(os.environ.get("NOTARI_FATIGUE_S", "4.0"))
FATIGUE_PAUSE_S: Final[float] = float(os.environ.get("NOTARI_FATIGUE_PAUSE", "3.0"))
...
class Prompter:
    ...
    One Prompter per notari session. Not thread-safe (intended for a
    ...
    def warn_fatigue(self) -> None:
        self.console.print(
            Panel(
                ...
                title="[yellow]notari · slow down[/yellow]",
                border_style="yellow",
            ),
        )
    ...
    def render_block(
        ...
        *,
        ...
        self.console.print(
            Panel(
                body,
                title=f"[bold red]notari · BLOCKED[/bold red]    [dim]{risk.value}[/dim]",
                border_style="red",
            ),
        )
    ...
    def _stdin_is_interactive(self) -> bool:
        ...
        Under `notari serve` the JSON-RPC reader owns stdin → input() would
    ...
    def _confirm_out_of_band(
        ...
        *,
        ...
        retries the same call after the user runs `notari approve <token>`,
        ...
        # terminal output.
        self.console.print(
            f"  [yellow]non-interactive · approve with:[/yellow] [bold]notari approve {token}[/bold]"
            if token
        ...
        decline_msg = (
            ...
            f"`notari approve {token}` (TTL 10m)"
            if token
    ...
    def confirm(
        ...
        *,
        ...
        self.console.print(
            Panel(
                body,
                title=f"[bold]notari[/bold]    [{risk_color}][{risk.value.upper()}][/{risk_color}]",
