"""Notari saves: rigorous, audit-log-grounded value accounting.
...
exercised in tests/test_saves_live.py with the actual ~/.notari log.
"""
...
#
# The blocked-reason field is free text like "Notari blocked: rm -rf /tmp/foo."
...
# pattern in src/notari/policy.py; if you add a new critical pattern there,
...
def _iter_events(log_path: Path) -> Iterator[dict[str, Any]]:
    ...
    silently; the chain has its own integrity check via `notari audit verify`,
...
def format_saves(saves: Saves, *, plain: bool = False) -> str:
    ...
    lines.append(b("Notari saves report") + f"  ({w_start} to {w_end})")
    lines.append(
        dim(
    ...
    # the "Notari saved you a click" outcome.
    from notari.severity import stat_line as _sl
    ...
    if saves.events_in_window == 0:
        ...
        lines.append("  notari insights        top patterns + suggested overrides")
        lines.append("  notari audit show      pretty-print recent events")
        lines.append("  notari audit export --pack    SOC 2 / EU AI Act PDF")
        if saves.biometric_approvals > 0:
            lines.append("  notari receipts show <session>    drill into a specific session")
