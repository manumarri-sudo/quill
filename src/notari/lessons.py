"""Notari Lessons: turn blocked AI PRs into lessons future agents learn from.
...
locally in ``.notari/mistakes.jsonl``. Repeated patterns aggregate into
...
MISTAKE_SCHEMA = "notari.mistake/v1"
...
def classify_path(path: str) -> str:
    ...
    lowered = path.lower()
    if path.startswith(".notari/") or path.startswith(".github/workflows/notari"):
        return "notari_trust_surface"
...
def mistakes_path(root: Path) -> Path:
    return root / ".notari" / "mistakes.jsonl"


def lessons_store_path(root: Path) -> Path:
    return root / ".notari" / "lessons.json"
...
def record_mistakes(passport: dict[str, Any], root: Path) -> int:
    ...
    Re-running `notari verify` on the same failing commit is idempotent; a new
...
_LESSONS: dict[tuple[str, str], tuple[str, str]] = {
    ...
    ("GATE_TAMPER", "*"): (
        "never-edit-notari-trust-surfaces",
        "Never edit Notari trust files, approver keys, perimeter files, or "
        "workflows that run Notari unless a human explicitly approves that "
...
_LESSON_META: dict[str, tuple[str, str]] = {
    ...
    "never-edit-notari-trust-surfaces": ("Notari trust surface or gate workflow touched", "block"),
        ...
        patterns.append(
            {
                ...
                "promote_command": f"notari lessons promote {lesson_id}",
            }
        )
