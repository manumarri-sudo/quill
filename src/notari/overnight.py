...
Active sources, evaluated in order:
  1. Manual toggle from `notari night` (highest priority, time-bounded)
...
State file: $NOTARI_HOME/overnight.json (mode 0o600).
"""
...
def _state_path() -> Path:
    ...
    return default_path("overnight.json", env_override="NOTARI_OVERNIGHT_FILE")
...
class OvernightState:
    ...
    `enabled`        True iff the user ran `notari night [on]` and the toggle
...
def is_manual_toggle_active(
    ...
    *,
    ...
    """True iff `notari night` was flipped on AND has not yet auto-expired.
...
def is_active(
    *,
    ...
    Sources of activation, in priority order:
      1. Manual toggle (`notari night`) - bounded by its expiry
...
def is_active_from_config(*, now: datetime | None = None) -> tuple[bool, str]:
    """Convenience wrapper: read [overnight] from NotariConfig + state file,
