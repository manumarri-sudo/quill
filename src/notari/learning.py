"""Autonomous learning loop. Notari reads its own audit data and updates
...
The non-negotiable architectural rules:
  ...
  4. Every update appends a timestamped line to ~/.notari/learning.log
...
# chance to confirm without having to run `notari suggestions promote`.
#
...
def _stats_path() -> Path:
    return Path(
        os.environ.get(
            "NOTARI_PATTERN_STATS",
            str(Path.home() / ".notari" / "pattern_stats.json"),
        )
    ).expanduser()
...
def _suggestions_path() -> Path:
    return Path(
        os.environ.get(
            "NOTARI_SUGGESTIONS",
            str(Path.home() / ".notari" / "suggestions.jsonl"),
        )
    ).expanduser()
...
def _log_path() -> Path:
    return Path(
        os.environ.get(
            "NOTARI_LEARNING_LOG",
            str(Path.home() / ".notari" / "learning.log"),
        )
    ).expanduser()
...
# ---------------------------------------------------------------------------
# Override reader. `notari suggestions promote` writes blocks into
...
def append_suggestion(payload: dict[str, Any]) -> None:
    ...
    `notari suggestions promote <id>` is the apply path."""
    p = _suggestions_path()
    p.parent.mkdir(parents=True, exist_ok=True)
...
def log_event(line: str) -> None:
    """Timestamped append to ~/.notari/learning.log. The operator tails
...
def detect_loosen_candidate(p: PatternStats) -> dict[str, Any] | None:
    ...
    `notari suggestions promote <id>` to write the override.
    """
    ...
    return {
        ...
        "proposal": (
            ...
            "`notari suggestions promote {id}`. Never auto-applied."
        ),
...
def detect_auto_promote_candidate(p: PatternStats) -> dict[str, Any] | None:
    ...
    `notari suggestions promote` command later.
    ...
    return {
        ...
        "proposal": (
            ...
            "Reverse anytime with `notari suggestions revoke`."
        ),
        "in_flow": True,
...
def post_decision_update(
    ...
    try:
        ...
        # Test-only env var NOTARI_LEARNING_STRICT raises; production stays soft.
...
_STALE_PATTERN_PREFIXES: tuple[str, ...] = ("approved one-shot via notari approve ",)
...
def find_stale_patterns(
    ...
    via notari approve <token-prefix>") rather than the original deny
...
def stats_summary(stats: dict[str, PatternStats] | None = None) -> dict[str, Any]:
    """Snapshot for `notari log` and dashboards. Pure read."""
    s = stats if stats is not None else load_stats()
    return {
        "n_patterns": len(s),
