...
from notari.policy import Scope, SessionIntent


@dataclass(slots=True)
class SubAgentExceedsParentScope(NotariError):
    """Raised when a child sub-agent declares a scope its parent doesn't have.

    Attenuation is one of the strongest guarantees Notari provides; violating
...
@dataclass(slots=True)
class SessionTree:
    """Owns the whole delegation tree for a single notari run.

    One per `notari serve` process. Hands out SessionNode handles and
