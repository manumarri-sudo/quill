...
`notari verify` trusts on-disk JSON (a contract, or a standing perimeter) to say
...
Where the trust root lives (escalating adversarial strength):
  ...
  1. ``.notari/approvers/*.pub`` - committed public keys. Convenient for a
     ...
     convenience layer.
  2. ``NOTARI_APPROVER_PUBKEYS`` env - newline-separated PEM blocks or file paths,
...
APPROVER_ENV = "NOTARI_APPROVER_PUBKEYS"


def approvers_dir(root: Path) -> Path:
    return root / ".notari" / "approvers"
    ...
    *,
    ...
    for raw_chunk in raw.replace(",", "\n\n").split("\n\n"):
        ...
        if "BEGIN" not in chunk and p.is_file():
            ...
            # `NOTARI_APPROVER_PUBKEYS=.notari/approvers/human.pub` would redirect
...
def load_trusted_approvers(
    ...
    trusted; the committed ``.notari/approvers/*.pub`` set is IGNORED. This closes
    ...
    if not trusted:
        return ProvenanceResult(
            ...
            (
                ...
                if strict
                else f"no trusted approver keys (add .notari/approvers/*.pub or set {APPROVER_ENV})"
            ),
            0,
        )
    if sig is None:
        return ProvenanceResult(
            ...
            None,
            "artifact is not signed; run `notari guard --key <approver.pem>` or `notari begin --key <approver.pem>`",
            len(trusted),
        )
