"""notari onboard - interactive first-run setup.

Replaces the placeholder-filled `notari init` flow with a guided 60-second
...
from notari.config import default_audit_path, default_config_path
            ...
            for child in ext_dir.iterdir():
                if "claude-dev" in child.name.lower() or "cline" in child.name.lower():
                    return DetectedAgent(
                        ...
                        label="Cline",
                        detected=True,
                        notes="use Notari's MCP proxy (v0.3 will ship a native adapter)",
                    )
...
def _detect_aider() -> DetectedAgent:
    if shutil.which("aider"):
        return DetectedAgent(
            ...
            label="Aider",
            detected=True,
            notes="use Notari's MCP proxy or wrap aider via shell function",
        )
...
def build_config_toml(
    *,
    ...
    parts: list[str] = [
        "# notari config - generated by `notari onboard`",
        "# https://github.com/manumarri-sudo/notari",
        "",
        "[session]",
        f'intent = "{intent}"',
...
def _prompt_choose_agents(
    ...
    if not found:
        console.print(
            ...
            "Notari can still gate any MCP-using tool via [bold]notari serve[/bold] "
            "(MCP proxy mode).",
        )
        return []
    chosen = []
...
def run(force: bool = False, console: Console | None = None) -> int:
    ...
    if not sys.stdin.isatty() and not force:
        out.print(
            ...
            "Run it in a real terminal, or use `notari init` for a non-interactive starter.",
        )
        return 2

    out.print(
        Panel.fit(
            "[bold]notari onboard[/bold]\n"
    ...
    out.print(
        Panel.fit(
            ...
            "  [dim]then run [bold]notari watch[/bold] for the live dashboard[/dim]\n"
            "  [dim]or [bold]notari audit show[/bold] to review what's been logged[/dim]\n"
            "  [dim]docs:[/dim] https://github.com/manumarri-sudo/notari",
            border_style="green",
        )
    )
    return 0
