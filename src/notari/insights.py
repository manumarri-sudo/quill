"""Notari insights: per-pattern analysis + tunable-recommendation surface.

Where `notari saves` shows aggregate counts ("Notari caught 32 rm -rf attempts"),
`notari insights` drills per-pattern:
...
This is the v0.3 surface for the "use the data to make Notari smarter"
...
from notari.saves import _in_window, _iter_events, _parse_ts, canonicalize_pattern
...
def format_insights(insights: Insights, *, plain: bool = False) -> str:
    ...
    lines.append(b("Notari insights") + f"  ({w_start} to {w_end})")
    lines.append(
        dim(
    ...
    # `notari saves`; the eye should land on the same colors with the same
    # meaning across surfaces.
    from notari.severity import color as _color
    from notari.severity import icon as _icon
    ...
    lines.append("  notari audit show --type verdict.blocked --last 30   recent blocks")
    if insights.downgrade_candidates:
        lines.append(
            "  notari trust add <path>                              promote a trust-path candidate"
        )
    if insights.reviewable_sessions:
        sid = insights.reviewable_sessions[0].session_id
        lines.append(
            f"  notari receipts show {sid[:12]}                drill into a flagged session"
        )
    lines.append(
        "  notari audit export --pack                           compliance PDF (SOC 2 / EU AI Act)"
    )
    return "\n".join(lines)
