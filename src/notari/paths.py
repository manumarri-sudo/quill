"""Resolve on-disk paths for Notari state.

A single NOTARI_HOME env var scopes everything (config, audit log, key,
permissions, telemetry, sessions, watch pid). Individual per-file env
vars still override (NOTARI_LOG, NOTARI_KEY, etc.) for surgical control.
Default home is ~/.notari.
"""
...
def notari_home() -> Path:
    return Path(os.environ.get("NOTARI_HOME", "~/.notari")).expanduser()


def default_path(filename: str, env_override: str | None = None) -> Path:
    """Resolve `<NOTARI_HOME>/<filename>`, with optional per-file override.

    If env_override is set and that env var is non-empty, use it directly.
    Otherwise fall back to <NOTARI_HOME>/<filename>.
    """
    if env_override:
        raw = os.environ.get(env_override, "").strip()
        if raw:
            return Path(raw).expanduser()
    return notari_home() / filename
