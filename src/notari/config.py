...
Notari config lives at ~/.notari/config.toml by default. Format:
...
from notari.policy import Risk, Scope
...
def default_config_path() -> Path:
    ...
    return default_path("config.toml", env_override="NOTARI_CONFIG")


def default_audit_path() -> Path:
    from notari.paths import default_path

    return default_path("audit.log.jsonl", env_override="NOTARI_LOG")


class UpstreamConfig(BaseModel):
    """One upstream MCP server that Notari proxies."""
    ...
    env_pass: list[str] = Field(
        ...
        description="Names of env vars to forward from Notari's environ "
        "(e.g. GITHUB_TOKEN). NEVER includes Notari's own secrets.",
    )
    ...
    def _check_env_pass(cls, v: list[str]) -> list[str]:
        # Refuse to forward anything that smells like Notari's signing key.
        forbidden = {"NOTARI_HMAC_KEY", "NOTARI_SIGNING_KEY"}
        bad = [e for e in v if e in forbidden]
        if bad:
            msg = f"env_pass cannot include notari secrets: {bad}"
...
class AuditConfig(BaseModel):
    model_config = ConfigDict(strict=True, extra="forbid")
    # If None, fall through to NOTARI_HOME-aware default at resolve time.
...
class OvernightConfig(BaseModel):
    ...
    Active when either:
      - `notari night` is manually toggled on (time-bounded, default 12h expiry)
...
def load_config(path: Path | None = None) -> NotariConfig:
    """Read TOML, validate, return a frozen-shape NotariConfig.
    ...
    if not p.exists():
        msg = f"no config at {p}. write one (see notari init) or set NOTARI_CONFIG."
        raise ConfigError(msg)
    try:
        with p.open("rb") as f:
            data = tomllib.load(f)
        return NotariConfig.model_validate(data)
...
def render_starter_config() -> str:
    """Return a starter TOML for `notari init`."""
    return """\
# notari config - see https://github.com/manumarri-sudo/notari
...
# Defaults to $NOTARI_HOME/audit.log.jsonl (or ~/.notari/audit.log.jsonl).
# Override per-project by setting `path` here or with NOTARI_LOG.
# path = "~/.notari/audit.log.jsonl"

# One [[upstream]] block per MCP server you want notari to proxy.
...
# regardless of trust. Manage with `notari trust add/remove/list/check`.
[trust]
paths = [
  # "~/projects/my-app",
  # "~/notari",
...
# WHAT TO TRY INSTEAD, and a one-shot `notari approve <token>` command.
# [notify]
# macos = true                          # macOS Notification Center banner
# sound = "Glass"                       # optional, macOS only
# email_to = "you@example.com"          # SMTP via [notify.email]; needs $NOTARI_SMTP_PASS
# slack_webhook_url = "https://hooks.slack.com/..."
# webhook_url = "https://your.endpoint/notari"  # generic JSON POST
...
# smtp_password_env = "NOTARI_SMTP_PASS" # password loaded from this env var
"""
