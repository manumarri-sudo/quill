"""Write promoted Quill lessons into agent instruction files, and render the
compact fix-prompt / agent-brief surfaces.

Managed-block contract: everything between the quill-lessons markers belongs
to Quill; everything outside is the user's and is never touched. Updates are
...
BLOCK_START = "<!-- quill-lessons:start -->"
BLOCK_END = "<!-- quill-lessons:end -->"
...
AGENT_TARGETS: dict[str, str] = {
    "claude": "CLAUDE.md",
    "codex": "AGENTS.md",
    "cursor": ".cursor/rules/quill-scope.mdc",
}
...
def render_block(promoted: list[dict[str, str]]) -> str:
    lines = [
        ...
        "## Quill agent lessons",
        "",
        "These rules come from prior Quill findings in this repo. Follow them "
...
def fix_prompt(passport: dict[str, Any], *, max_findings: int = 5) -> str:
    ...
    if verdict == "PASS":
        return "Quill passed this change; nothing to fix."

    recs = build_remediations(passport)
    lines = [
        f"Quill {'blocked' if verdict == 'BLOCK' else 'flagged'} this PR. "
        f"Fix ONLY the findings below — do not weaken, bypass, or edit Quill's "
    ...
    if len(recs) > max_findings:
        lines.append(
            f"- …and {len(recs) - max_findings} more (run `quill explain` for the full list)."
        )
    lines += [
        "",
