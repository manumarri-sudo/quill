# Notari Lessons: local learning for AI coding agents

Notari does not train a model. It turns repeated local findings into compact,
human-approved instructions for the agents that work in *this* repo.

When `notari verify` returns BLOCK or NEEDS_REVIEW, Notari records a small
structured note about what went wrong. When the same kind of mistake repeats,
`notari lessons` surfaces it and suggests a one-line rule. A human promotes the
rules they agree with, and `notari teach` writes them into the agent instruction
...
- **Local by default.** Mistake events live in `.notari/mistakes.jsonl` and
  promoted lessons in `.notari/lessons.json`, both inside your repo. Nothing is
...
- **Human-gated.** No lesson is ever auto-promoted or auto-written into an
  instruction file. `notari lessons promote <id>` is an explicit human action,
  and `notari teach` only writes lessons you already promoted.
- **Advisory, not policy.** A promoted lesson is guidance an agent reads; it
  does **not** change what Notari blocks. The signed perimeter and contract are
...
```bash
notari lessons                                 # repeated mistakes, ranked, with a suggested lesson + severity
notari lessons --json                          # machine-readable
notari lessons promote no-ci-edits-without-ci-scope   # accept one (idempotent)
notari teach --agents claude,codex,cursor      # write promoted lessons into agent instruction files
notari agent-brief                             # compact pre-work brief: task, scope, forbidden paths, lessons
```

`notari teach` edits only the block between the `<!-- notari-lessons:start -->`
and `<!-- notari-lessons:end -->` markers. Everything you wrote outside that block
...
| `block` | Notari already blocks this deterministically today. |
...
- **Recording is concurrency-safe.** Each `notari verify` serializes its mistake
  records into a single atomic `O_APPEND` write, so parallel agents can't
  interleave or corrupt a line in `.notari/mistakes.jsonl`.
...
- **Teaching is a human, single-writer step.** `notari teach` is something an
...
- **One boundary, many agents.** The contract and perimeter are per-repo and
  ...
  boundary. Hand each agent the output of `notari agent-brief` before it starts so
